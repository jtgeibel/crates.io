        ...
        .map(|dep| {
            if let Some(registry) = &dep.registry {
                if !registry.is_empty() {
                    return Err(ErrorBuilder::custom_cargo_err_legacy(format!("Dependency `{}` is hosted on another registry. Cross-registry dependencies are not permitted on crates.io.", &*dep.name)));
                }
            }
            ...
            let krate = Crate::by_exact_name(&dep.name)
                .first::<Crate>(&*conn)
                .map_err(|_| ErrorBuilder::custom_cargo_err_legacy(format!("no known crate named `{}`", &*dep.name)))?;
